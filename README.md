...
This repository contains the source code for the Food Ordering Project, a web application that enables users to browse menu items, add them to a cart, and place orders. Admin users can manage menu items, track orders, and view reports.

## Project Responsibilities

### Wenzhe Wang
- **User Authentication**: Implemented login, logout, and registration functionalities for both users and admins.
- **Menu & Product Management**: Developed features to allow admin users to create, update, and delete menu items and products.
- **Order Processing & Management**: Enabled users to place orders, view order history, and track order statuses. Admins can update and manage these orders.
- **Reporting & Analytics**: Created the admin dashboard that provides insights on sales, order volumes, and other business metrics.

### Phoi Nghiem
- **User Profile Management**: Designed and implemented functionalities allowing users to view and update their profile details, such as contact information and passwords.
- **Cart Management**: Developed features for users to add items to their cart, adjust quantities, and remove items before placing an order.
- **Payment Integration**: Integrated secure payment gateways for credit card payments and added support for Cash on Delivery (COD) options.
- **Contact & Feedback Management**: Created a system that enables users to send feedback or queries through a contact page, which gets directed to admin.
...
- **NuGet Packages** required for the project (managed via `packages.config`).
...
- Create a new database named FoodOrderingDB or update the database name in the connection string.
...
END
```
- Run the SQL scripts located in the App_Data folder to create the necessary tables and seed data.

5. **Build and Run the Project**
- Build the solution using Build > Build Solution
- Set the Food_Ordering_Project as the startup project.
